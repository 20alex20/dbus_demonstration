...
#define DBUS_ERROR(str)                                                        \
  throw sdbus::Error(                                                          \
      sdbus::Error::Name{"com.system.configurationManager.Error"}, str);
...
void generateObject(sdbus::IConnection *connection, const std::string &destDir,
                    const std::string &applicationName) {
  sdbus::ObjectPath objectPath{"/com/system/configurationManager/Application/" +
                               applicationName};
  auto app =
      new application{sdbus::createObject(*connection, std::move(objectPath)),
                      applicationName,
                      {}};
  ...
  auto changeConfiguration = [app, &destDir](const std::string &key,
...
int main() {
  ...
  std::string destDir = std::string(getpwuid(getuid())->pw_dir) +
  ...
  for (const auto &entry : fs::directory_iterator(destDir)) {
    ...
    std::string applicationName =
        path.substr(indexSlashPlus, indexDot - indexSlashPlus);
    try {
      generateObject(connection.get(), destDir, applicationName);
    } catch (const sdbus::Error &e) {
      std::cerr << "Error creating dbus-object for application \""
